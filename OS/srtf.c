...
struct process{
    int arriv,burst,compl,wait,tat,tempburst;
};
...
int main(){
    ...
    for(int i=0;i<=sum;i++){
        int minburst=9999,flag=0,pnum=0;
        for(int j=0;j<n;j++){
            if(p1[j].burst>0&&currtime>=p1[j].arriv&&minburst>p1[j].burst) {minburst=p1[j].burst;flag=1;pnum=j;}
        }
        if(!flag) currtime++;
        if(flag){
            p1[pnum].burst--;
