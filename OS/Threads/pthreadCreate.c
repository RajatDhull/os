...
int i;
 ...
 void *thread_function4(void *arg){
 	...
 	if(arg[0]>max) max=arg[0];
 	if(arg[1]>max) max=arg[1];
 	if(arg[2]>max) max=arg[2];
 	printf("Among %d,%d,%d, %d is the largest\n",arg[0],arg[1],arg[2],max);
...
pthread_t thread1,thread2,thread3,thread4;  //thread declaration
 ...
 pthread_join(thread1, &res1); //process waits for thread to finish, store the argument of pthread_exit in res
